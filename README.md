...
A tool for reviewing websites. Pick a project (a website URL), see the site in a frame, click any element, write a comment – it stays anchored to that element like a tracked change in Word. Everyone sees all comments; users edit only their own; admins manage projects and export.
...
- User: pick a project, comment, see everybody's comments, edit / delete own
...
GET    /api/comments?project=ID[&path=/x]        every comment of the page; items carry author {name,email} and mine
POST   /api/comments                             { project_id, page_path, body, selector, xpath, text_snippet, tag_name, rect_top }
GET    /api/comments/:id
PATCH  /api/comments/:id                         own only (admin: any)   { body }
DELETE /api/comments/:id                         own only (admin: any)
```
...
src/
  ...
  lib/projects.ts, comments.ts, export.ts, url.ts (SSRF guard)
...
tests/                     vitest: proxy rewriting, auth, URL validation
